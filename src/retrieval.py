...
class VectorRetriever:
    def __init__(self, vector_db_dir: Path, documents_dir: Path, embedding_provider: str = "agicto"):
        # 初始化向量检索器，加载所有向量库和文档
        self.vector_db_dir = vector_db_dir
        self.documents_dir = documents_dir
        self.all_dbs = self._load_dbs()
        # 默认使用 agicto 作为 embedding provider（AGICTO OpenAI 兼容接口）
    ...
    def _get_embedding(self, text: str):
        ...
        if self.embedding_provider == "openai":
            ...
            # 通过 AGICTO 统一出口获取嵌入（text-embedding-v4）：
            # 带 Redis 缓存（命中直接返回，P99 < 50ms）与固定窗口限流，Redis 异常自动降级
            from src.api_requests import get_embeddings
            embedding = get_embeddings([text], model="text-embedding-v4")[0]
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        target_report = None
        # 第一遍：company_name 精确匹配（如年报 company_name="中芯国际"），
        # 精确匹配优先级最高，避免被文件遍历顺序中更早出现的 file_name 模糊匹配遮蔽
        for report in self.all_dbs:
            metainfo = report.get("document", {}).get("metainfo", {})
            if metainfo.get("company_name") == company_name:
                target_report = report
                break
        # 第二遍：精确匹配未命中时，fallback 到 file_name 包含关系
        # （如券商研报 company_name 为券商名，但文件名中含所研究公司名）
        if target_report is None:
            for report in self.all_dbs:
                metainfo = report.get("document", {}).get("metainfo", {})
                if company_name in metainfo.get("file_name", ""):
                    target_report = report
                    break
        if target_report is None:
