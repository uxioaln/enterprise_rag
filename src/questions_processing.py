...
class QuestionsProcessor:
    ...
    def _retrieve_for_company(self, company_name: str, question: str) -> list:
        """单公司检索：返回 retrieval_results（含 page/text/distance）。
        从原 get_answer_for_company 抽取的检索步骤，逻辑保持一致。"""
        t0 = time.time()
    ...
    def _generate_answer_from_results(self, question: str, retrieval_results: list, schema: str, company_name: str) -> dict:
        """根据检索结果生成答案：格式化上下文 -> LLM -> 页码校验 -> 引用提取。
        从原 get_answer_for_company 抽取的生成步骤，逻辑保持一致。"""
        t4 = time.time()
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        # 重构后内部复用 _retrieve_for_company 与 _generate_answer_from_results，逻辑与原实现一致
        t0 = time.time()
        retrieval_results = self._retrieve_for_company(company_name, question)
        answer_dict = self._generate_answer_from_results(question, retrieval_results, schema, company_name)
        print(f"[计时] [get_answer_for_company] 总耗时: {time.time()-t0:.2f} 秒")
        return answer_dict

    def retrieve_question_contexts(self, question: str) -> tuple:
        """单条问题检索（不调用 LLM），返回检索结果与路由信息。
        复用 process_single_question 的公司抽取与路由逻辑。
        返回: (retrieval_results, company_name, is_comparative, companies)
          - 单公司: (retrieval_results, company_name, False, None)
          - 多公司: (聚合的 retrieval_results, None, True, [公司名列表])
        """
        if self.new_challenge_pipeline:
        ...
        if len(extracted_companies) == 1:
            ...
            retrieval_results = self._retrieve_for_company(company_name, question)
            return retrieval_results, company_name, False, None

        # 多公司比较：聚合各公司检索结果作为上下文
        aggregated: list = []
        for company_name in extracted_companies:
            try:
                results = self._retrieve_for_company(company_name, question)
                aggregated.extend(results)
            except Exception as e:
                print(f"[retrieve_question_contexts] 公司 {company_name} 检索失败: {e}")
        return aggregated, None, True, extracted_companies
    ...
    def generate_answer_from_contexts(
        ...
        # 公司名：优先使用传入值，否则从问题中抽取
        if company_name is None:
            if self.new_challenge_pipeline:
                extracted = self._extract_companies_from_subset(question)
            else:
                extracted = re.findall(r'"([^"]*)"', question)
            company_name = extracted[0] if extracted else None

        if self.new_challenge_pipeline and company_name:
            pages = answer_dict.get("relevant_pages", [])
            if isinstance(pages, list) and retrieval_results:
                validated_pages = self._validate_page_references(pages, retrieval_results)
            else:
    ...
    def _post_process_submission_answers(self, processed_questions: List[dict]) -> List[dict]:
        ...
        for q in processed_questions:
            ...
            if value == "N/A":
                ...
                references = [
                    {
                        "pdf_file_name": ref["pdf_file_name"],
    ...
    def process_comparative_question(self, question: str, companies: List[str], schema: str) -> dict:
        ...
        for ref in aggregated_references:
            key = (ref.get("pdf_file_name"), ref.get("page_index"))
            unique_refs[key] = ref
