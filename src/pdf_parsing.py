...
# docling 为可选依赖：运行时问答服务（FastAPI / Celery）不使用 docling，
# 仅离线 PDF 解析脚本（PDFParser）需要。缺失时模块仍可正常导入，
# 真正实例化 PDFParser 解析 PDF 时才抛出明确错误
try:
    ...
    _log.warning("未检测到 docling 依赖，PDFParser 离线解析功能不可用；运行时问答服务不受影响")
...
class PDFParser:
    def __init__(
        ...
        # 未显式传入 backend 时沿用 docling 默认 backend；docling 缺失时
        # 由 _create_document_converter 抛出明确错误（保留原有默认行为）
    ...
    def _create_document_converter(self) -> "DocumentConverter": # type: ignore
        ...
        if not DOCLING_AVAILABLE:
            raise RuntimeError(
                "PDFParser 依赖 docling 进行 PDF 解析，当前环境未安装 docling。"
                "如需离线解析 PDF，请先安装 docling（pip install docling）后重试。"
            )
