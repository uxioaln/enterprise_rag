...
# --------------------------------------------------------------------------- #
# 低置信度自动查询改写重试：评估与改写 Prompt
# --------------------------------------------------------------------------- #
class RetrievalEvaluationPrompt:
    """检索质量与答案置信度评估 Prompt（占位，待填充具体文案）。

    要求模型按指定 JSON Schema 输出置信度评分与反思内容，供重试循环决策。
    """

    instruction = """
你是一个 RAG（检索增强生成）质量评估专家。
你的任务是评估给定问题、检索到的上下文以及生成的答案的整体质量。
请从以下维度进行评分（0-1，步长0.1）：
1. retrieval_confidence：检索结果与问题的相关性与充分性
2. faithfulness：答案是否完全由上下文支撑，是否存在幻觉
3. completeness：答案是否完整回答了问题的全部要点

overall 为三者的加权综合得分（建议 retrieval 0.3 + faithfulness 0.4 + completeness 0.3）。
critique 为简要反思说明：指出当前答案的不足之处，以及下一次查询改写应关注的方向。
should_retry：overall 低于阈值时为 true，表示需要重试。
"""

    class EvaluationSchema(BaseModel):
        """评估结果结构定义。"""
        overall: float = Field(description="综合置信度（0-1）")
        retrieval_confidence: float = Field(description="检索结果置信度（0-1）")
        faithfulness: float = Field(description="答案忠实度（0-1）")
        completeness: float = Field(description="答案完整性（0-1）")
        critique: str = Field(description="简要反思说明，指出不足与改写方向")
        should_retry: bool = Field(description="是否需要重试")

    pydantic_schema = re.sub(r"^ {4}", "", inspect.getsource(EvaluationSchema), flags=re.MULTILINE)
...
class QueryRewritePrompt:
    """查询改写 Prompt（占位，待填充具体文案）。

    要求模型根据评估反思对原始查询进行改写，以提升下一轮检索质量。
    """

    instruction = """
你是一个查询改写专家。
你的任务是根据评估反馈，对原始问题进行改写，以提升下一轮检索的质量。

改写策略：
- expand：扩展查询，补充关键词或限定条件
- refine：细化查询，聚焦更具体的信息需求
- decompose：拆分查询，将复杂问题分解为更简单的子问题
- rephrase：重述查询，用不同的表述方式表达相同意图

改写要求：
1. 保持原始问题的核心意图不变
2. 基于评估反思中的方向进行调整
3. 改写后的问题应能被检索系统更好地匹配
4. 若改写结果与当前查询过于相似，强制追加限定词以避免死循环
"""

    class RewriteSchema(BaseModel):
        """改写结果结构定义。"""
        rewritten_query: str = Field(description="改写后的查询")
        rewrite_strategy: str = Field(description="改写策略：expand/refine/decompose/rephrase")
        rationale: str = Field(description="改写理由，说明为何选择该策略")

    pydantic_schema = re.sub(r"^ {4}", "", inspect.getsource(RewriteSchema), flags=re.MULTILINE)
